...
async function startServer() {
    ...
    const rooms = new Map();
    const wsRateLimiters = new WeakMap();
    wss.on("connection", (ws) => {
        let currentRoom = null;
        const limiter = new RateLimiter();
        wsRateLimiters.set(ws, limiter);
        ws.on("message", (data) => {
            if (!limiter.isAllowed("message")) {
            ...
            try {
                ...
                if (message.type === "join") {
                    ...
                    if (!roomId || typeof roomId !== 'string' || !roomId.match(/^[a-zA-Z0-9_-]{1,100}$/)) {
                ...
                if (message.type === "vibe" && currentRoom) {
                    ...
                    rooms.get(currentRoom)?.forEach((client) => { if (client !== ws && client.readyState === WebSocket.OPEN)
                        client.send(JSON.stringify({ type: "remote_vibe", text })); });
                }
            }
            catch (e) {
    ...
    app.post("/api/webhook/:roomId", validateWebhookAuth, (req, res) => {
        ...
        if (roomClients) {
            roomClients.forEach((client) => { if (client.readyState === WebSocket.OPEN)
                client.send(JSON.stringify({ type: "remote_vibe", text: message })); });
            res.json({ status: "ok", sentTo: roomClients.size });
        }
        else

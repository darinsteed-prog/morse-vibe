...
export default function App() {
  ...
  const modeLabels = { type: 'Keys', tap: 'Tap', img: 'Img', qr: 'QR', sound: 'Sound', flash: 'Flash', remote: 'Remote', atc: 'Air', decipher: 'Fix', ref: 'Guide', help: 'Help' };
  return (
    <div className={`min-h-screen flex flex-col max-w-md mx-auto p-6 relative transition-colors duration-75 ${isVisualActive ? 'bg-vibe-primary/20' : 'bg-vibe-bg'}`}>
      ...
      <main className='flex-1 flex flex-col gap-4'>
        ...
        {inputMode === 'type' ? (<KeyboardMode text={text} setText={setText} isTransmitting={isTransmitting} clearInput={clearInput} />) : inputMode === 'tap' ? (<TelegraphMode text={text} tapSequence={tapSequence} isTransmitting={isTransmitting} handlePointerDown={handlePointerDown} handlePointerUp={handlePointerUp} clearInput={clearInput} />) : inputMode === 'img' ? (<ImageMode setText={setText} isTransmitting={isTransmitting} />) : inputMode === 'qr' ? (<QRMode setText={setText} isTransmitting={isTransmitting} />) : inputMode === 'sound' ? (<SoundMode text={text} isTransmitting={isTransmitting} />) : inputMode === 'flash' ? (<FlashMode text={text} isTransmitting={isTransmitting} />) : inputMode === 'decipher' ? (<DecipherMode />) : inputMode === 'ref' ? (<ReferenceMode />) : inputMode === 'help' ? (<HelpMode />) : inputMode === 'atc' ? (<ProGate feature="Air Radar" description="Live flight radar with aircraft analysis. Unlock Pro to access."><ATCMode /></ProGate>) : (
